  ...
  return <Container py={"lg"} size={720}>
    ...
    <CompanyExperience company="Enzo Health" location="Lehi, UT" >
      ...
      <BulletPoints bullets={[
        ...
        "Architected Enzo's workflow engine after recognizing that AI extraction alone wasn't enough — clinicians still had to manually transfer results into their EHR. Built a DAG-based orchestration system with an event-driven executor: referrals arrive from integrated sources (referral portals, fax, email), trigger the document pipeline, and flow through a graph of automated and human-in-the-loop nodes — with automatic fallbacks that degrade gracefully (e.g., failed insurance verification spawns a human task with the UI context needed to complete it manually). Once a patient is accepted, the workflow builds the patient record directly in the customer's EHR. Built a drag-and-drop workflow configurator so a technical team can onboard new customers with zero code changes, wiring together node types with typed inputs and outputs to match each customer's intake operations.",
      ]} />
    </CompanyExperience>
